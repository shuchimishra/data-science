...
# as well as the subdirectory where the fiels are
mainDir1 <- "C:/Users/MKevane"
mainDir2 <- "C:/Users/Michael Kevane"
subDir <- "/Google Drive/Classes/ECON 41-42 for MK/files42/data"
...
qplot(deny, pirat,  geom=c("boxplot"), data=subset(HMDA, pirat<1),
      main="P/I ratio by whether denied a loan", xlab="Was denied a loan?", ylab="P/I ratio")

# A scatter plot is not very informative- note convert outcome deny to numeric 0-1
qplot(pirat, as.numeric(deny)-1, data=HMDA,
      main="Denial by P/I ratio excluding outliers", xlab="P/I ratio", ylab="Deny")

# But the relationship is positive...  note linear model is questionable
qplot(pirat, as.numeric(deny)-1, geom=c("point","smooth"), 
      data=HMDA,
      main="Denial by P/I ratio excluding outliers", xlab="P/I ratio", ylab="Deny")
